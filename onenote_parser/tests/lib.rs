...
#[test]
fn test_parse_2016_notebook() {
    let parser = Parser::new();
    let notebook = parser
        .parse_section(Path::new("tests/samples/onenote-2016/OneWithFileData.one"))
        .unwrap();

    assert!(!notebook.page_series().is_empty());
    assert_debug_snapshot!(notebook);
}
