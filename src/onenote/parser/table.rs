...
pub(crate) fn parse_table(table_id: ExGuid, rev: &Revision, store: &OneStore) -> Table {
    let table_object = rev
        .resolve_object(table_id, store)
...
fn parse_row(row_id: ExGuid, rev: &Revision, store: &OneStore) -> TableRow {
    let row_object = rev
        .resolve_object(row_id, store)
    ...
    let contents = data
        .cells()
        .iter()
        .map(|id| parse_cell(*id, rev, store))
        .collect();

    TableRow { contents }
}

fn parse_cell(cell_id: ExGuid, rev: &Revision, store: &OneStore) -> TableCell {
    let cell_object = rev
        .resolve_object(cell_id, store)
    ...
    let contents = data
        .contents()
        .iter()
        .map(|id| parse_outline_element(*id, rev, store))
        .collect();
